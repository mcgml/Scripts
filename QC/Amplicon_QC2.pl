...
while (<$SS_in>) {
    ...
    if ($n == 1) { #now on [manifest]
        ...
        $s++;
        ($SampleID, $SampleName, $str) = split(',', $_);
        
        if ($SampleName ne '') {
            push (@SampleIDs, $SampleName . '_S' . $s);
        } else {
            push (@SampleIDs, $SampleID . '_S' . $s);
        }
    }
...
foreach $SampleID (@SampleIDs){
    ...
    $filename = $SampleID . '.bam';
    
    #collect samtools output
    $stat = `"C:\\Illumina\\MiSeq Reporter\\Workflows\\PcrAmpliconWorker\\samtools.exe" flagstat $filename`;
    chomp $stat;
    ...
    if ($j == 1) {
        ...
        foreach $str (keys %regions){
            $frequency = `"C:\\Illumina\\MiSeq Reporter\\Workflows\\PcrAmpliconWorker\\samtools.exe" view -c $filename $regions{$str}`; 
            chomp $frequency;
